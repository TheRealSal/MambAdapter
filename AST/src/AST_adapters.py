...
class Bottleneck_adapter(nn.Module):
    def __init__(self, in_dim, reduction_rate, out_dim):
        ...
        nn.init.zeros_(self.linear_downsample.weight);
...
class ASTLayer_adapter(ASTLayer):
    def __init__(self, config, adapter_config):
        ...
        self.adapter_module_FFN = self.make_adapter(config.hidden_size, adapter_config.REDUCTION_RATE,
                                                    config.hidden_size, adapter_config.ADAPTER_BLOCK,
                                                    adapter_config.KERNEL_SIZE, adapter_config.MAMBA_CONFIG)

        if adapter_config.ADAPTER_TYPE == 'Houlsby':
            self.adapter_module_MHSA = self.make_adapter(config.hidden_size, adapter_config.REDUCTION_RATE,
                                                         config.hidden_size, adapter_config.ADAPTER_BLOCK,
                                                         adapter_config.KERNEL_SIZE, adapter_config.MAMBA_CONFIG)
    ...
    def make_adapter(self, in_dim, reduction_rate, out_dim, adapter_block, kernel_size, mamba_config):
        if adapter_block == 'bottleneck':
            ...
            return adapter_layer
        elif adapter_block == 'mambadapter':
            adapter_layer = MambAdapter(in_dim, reduction_rate, kernel_size, mamba_config)
            return adapter_layer
...
class AST_adapter(nn.Module):
    def __init__(self, max_length: int, num_classes: int, final_output: str, reduction_rate: int, adapter_type: str,
                 seq_or_par: str, apply_residual: bool, adapter_block: str, kernel_size: int, mamba_config: dict,
                 model_ckpt="MIT/ast-finetuned-audioset-10-10-0.4593", cache_dir=None):
        ...
        self.adapter_config = Adapter_config(reduction_rate, adapter_type, seq_or_par, apply_residual, adapter_block,
                                             kernel_size, mamba_config)

        self.model = ASTModel_adapter.from_pretrained(model_ckpt, self.adapter_config, max_length=max_length,
        ...
        assert final_output in ['CLS', 'ALL'], (
            "Classification can be only applied to the [CLS] token or to the entire sequence!")
...
class ASTEncoder_adapter_ablation(ASTEncoder):
    def __init__(self, config, adapter_config):
        ...
        self.layer = nn.ModuleList(
            [ASTLayer_adapter_ablation(config, adapter_config) for _ in range(config.num_hidden_layers)])
...
class ASTLayer_adapter_ablation(ASTLayer):
    def __init__(self, config, adapter_config):
        ...
        self.adapter_module = self.make_adapter(config.hidden_size, adapter_config.REDUCTION_RATE, config.hidden_size,
                                                adapter_config.ADAPTER_BLOCK, adapter_config.KERNEL_SIZE)
        self.adapter_config = adapter_config
...
class AST_adapter_ablation(nn.Module):
    def __init__(self, max_length: int, num_classes: int, final_output: str, reduction_rate: int, seq_or_par: str,
                 location: str, adapter_block: str, before_after: str, kernel_size: int,
        ...
        self.adapter_config = Adapter_config_ablation(reduction_rate, seq_or_par, before_after, location, adapter_block,
                                                      kernel_size)
        self.model = ASTModel_adapter_ablation.from_pretrained(model_ckpt, self.adapter_config, max_length=max_length,
